...
> 当生成约束足够密集时，模型会以什么方式失控？

为了研究这个问题，项目选择了古典词牌《鹧鸪天》作为“受控压力场”。

因为要生成一首合格的《鹧鸪天》，模型必须同时满足：
...
* 会为了语义放弃押韵，
* 会出现意象失联，
* 还有一些推理模型，会把所有 token 都耗费在“思考”上，最后甚至没来得及真正写完作品。
...
当前内置 5 个不同压力梯度的《鹧鸪天》测试样本，用于观察不同模型在高约束生成中的失稳模式。
...
一旦模型某一步生成漂移，后续几乎没有“修复空间”。

这意味着：

> 我们可以非常清晰地观察模型是在哪一步开始失稳的。
...
当前沿模型在标准 benchmark 上越来越接近时，真正的差异开始出现在：
...
> 模型内部出现了“语义优先级”与“形式约束”之间的竞争。

---

## 案例 2：DeepSeek 的“思考过载”

某次实验中，DeepSeek 推理模型把几乎所有 token 都用于内部推理：
...
本项目推荐使用 [uv](https://github.com/astral-sh/uv) 进行环境管理与可复现实验运行。
...
├── rules/
│   └── zhegutian_zhengti.json # 《鹧鸪天》正体格律规则
├── src/
│   ├── metrics/
│   │   ├── antithesis.py    # 对仗检查（LLM）
│   │   ├── pingze.py        # 平仄检查（规则）
│   │   ├── rhyme.py         # 押韵检查（中华新韵）
│   │   └── semantic.py      # 语义/意境检查（LLM）
│   ├── schema/
│   │   └── rule_config.py   # 规则结构定义
│   ├── evaluator.py         # 评测调度（100分制，5维度）
│   ├── generator.py         # 大模型生成（策略+工厂模式）
│   ├── llm_client.py        # 统一 LLM 调用客户端
│   ├── main.py              # 程序入口
│   └── utils.py             # 文本清洗、断句、拼音提取
├── tools/
